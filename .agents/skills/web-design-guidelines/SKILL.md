...
1. Fetch fresh guidelines from the source URL below (or refer to the offline rules reference below)
2. Read the specified files (or prompt user for files/pattern)
3. Check against all rules in the guidelines
4. Output findings in concise, high signal-to-noise `file:line` format
...
- Interactive elements need keyboard handlers (`onKeyDown`/`onKeyUp`/`keydown`)
...
### Focus States
- Interactive elements need visible focus: `focus-visible:ring-*` or equivalent focus styles
...
- Labels clickable (`htmlFor`/`for` or wrapping control)
